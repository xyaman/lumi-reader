...
function Navbar(props: SectionProps) {
    ...
    return (
        <nav
            class={`navbar-theme${isFixed ? " fixed" : ""} top-0 left-0 w-full h-14 backdrop-blur-md shadow-md px-4 z-30 flex items-center justify-between transition-all duration-300`}
        >
            {props.children}
        </nav>
    )
